...
namespace domain {
Item::Item(const item::Name& name, const Level& level, const Attack& attack,
           const Defense& defense, const item::Category& category,
           const item::BuyPrice& buy_price, const item::SellPrice& sell_price)
    : name_(name),
      level_(level),
      attack_(attack),
      defense_(defense),
